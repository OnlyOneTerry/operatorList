...
QPainterPath EditorLineItem::shape() const
{
    //添加鼠标选中宽度
    ...
    QPainterPath path;
    path.moveTo(/*mapToScene*/(m_line[BEGIN]));
    path.lineTo(/*mapToScene*/(m_line[END]));
...
void EditorLineItem::updateLineFLength()
{
    //
    QLineF line(m_line[BEGIN],m_line[END]);
    qreal lineLength = line.length();//qSqrt ( (line.dx()*line.dx())+(line.dy()*line.dy()));
    //求边长
    //    QPointF dist = m_line[BEGIN] - m_line[END];
    EditorPublicClass::init()->DoubleSpinBox->setValue(lineLength);
}
...
void EditorLineItem::toXml(QDomElement &elt) const
{
    ...
    elt.setAttribute( "style", IOHelper::variantToString( (int)m_item->pen().style() ) );
