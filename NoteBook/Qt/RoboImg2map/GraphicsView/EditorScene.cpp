...
{
    SCDebug;
    delete m_groupUnderlay;
    delete m_groupOverlay;
    delete m_factory;
}
...
{
    qDebug()<<"m_groupUnderlay size is -------"<<m_groupUnderlay->childItems().size();
    qDebug()<<"m_groupOverlay size is -------"<<m_groupOverlay->childItems().size();
...
void EditorScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    ...
    if (m_modeEdition == BaseItem::EDITION_NEXTITEM)
    {
        ...
        m_primitive = Q_NULLPTR; // in order to change pen or brush after creation
...
{
    qDebug()<<"EditorScene::keyPressEvent";
...
void EditorScene::setMode(const QString &m)
{
    ...
    if (m_typElem == "Select")
    {
        ...
        emit sigCusor(m,QCursor(Qt::ArrowCursor));
        changeDragMode(QGraphicsView::ScrollHandDrag);
    ...
    else
    {
        ...
        //        qDebug()<<"QApplication::setOverrideCursor( QCursor(Qt::CrossCursor) )";
        emit sigCusor(m,QCursor(Qt::CrossCursor));
...
QImage EditorScene::getSelectionImage()
{  
    QImage  image;
    // cropRect is in scene coord
    foreach ( QGraphicsItem * item, m_groupUnderlay->childItems() )
    {
        QGraphicsPixmapItem* pixItem = Q_NULLPTR;
        if ( ( pixItem = qgraphicsitem_cast<QGraphicsPixmapItem*>(item) ) != Q_NULLPTR )
        {
            QRect itemCropRect = getSelectionRect().toRect();

            if(!itemCropRect.isEmpty()){
                QRect pixRect = pixItem->pixmap().rect().intersected( itemCropRect );
//                SCDebug;
                image =  pixItem->pixmap().toImage().copy(pixRect);
            }else{
                image =  (pixItem->pixmap().toImage().copy());
//                SCDebug;
            }
            break;
        }
...
QRectF EditorScene::getSelectionRect() const
{
    QRectF rect(0,0,0,0);
    ...
    if ( !items.isEmpty() )
    {
        ...
        {
            BaseItem *primitive = dynamic_cast<BaseItem *>(item);
            if (primitive->getType()=="EditorRectangleItem")
            {
                rect = primitive->boundingRect();
                qDebug()<<"rect1:"<<rect;
                break;
            }
        }
